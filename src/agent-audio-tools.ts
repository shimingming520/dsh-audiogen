...
export function registerAgentAudioTools(ctx: Context, resolve: () => AgentAudioToolConfig): () => void {
  const disposer = ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      format: { type: 'string', description: 'Output format. MiniMax music: mp3/wav/pcm. ElevenLabs sound effects: codec name mp3/pcm/ulaw/alaw/opus (combined with sample_rate and bitrate into the single output_format codec_sample_rate_bitrate, e.g. mp3_22050_32; pcm/ulaw/alaw carry no bitrate).' },
      ...
      sample_rate: { type: 'integer', description: 'Sample rate in Hz. MiniMax: music 16000/24000/32000/44100 (default 44100), tts default 32000 (audio_setting.sample_rate). ElevenLabs sound effects: 8000/16000/22050/24000/32000/44100/48000, combined with format and bitrate into the single output_format codec_sample_rate_bitrate.' },
      bitrate: { type: 'integer', description: 'Bitrate. MiniMax in bps: 32000/64000/128000/256000 (music default 256000, tts default 128000). ElevenLabs sound effects in kbps: 32/48/64/96/128/192 (mp3/opus only; 192 kbps needs Creator plan or above), combined with format and sample_rate into the single output_format codec_sample_rate_bitrate.' },
        ...
        if (action === 'recommend') {
          ...
          void appendVoiceRecommendRecord({
            ...
            recommendations: recommendations.map(item => ({
              ...
              ...(item.descriptive === undefined ? {} : { descriptive: item.descriptive }),
