	...
	const managementDisposer = ctx.tools.register(defineTool({
		...
		parameters: {
			action: {
				...
				required: true,
				description: "list = browse/filter voices; recommend = pick voices for a requirement with the agent default model; delete = remove an owned voice."
			},
			channel: {
				type: "string",
			...
			language: {
				type: "string",
				description: "Filter for list/recommend: language substring (ISO code like en/zh/ja, or a label like Chinese (Mandarin))."
			},
			keyword: {
				type: "string",
			...
			use_case: {
				type: "string",
				description: "Official filter: use case, e.g. characters_animation / conversational / narration / gaming (ElevenLabs shared library)."
			},
			accent: {
				type: "string",
				description: "Official filter: accent, e.g. british / american / australian."
			},
			gender: {
				type: "string",
