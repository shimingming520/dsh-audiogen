	...
	factory: (require) => {
		...
		*/
		...
		var AudiogenApi = class {
			...
			async voiceList(request) {
				const body = await (await postJson(VOICES_API.list, request)).json();
				return {
					ok: body.ok === true,
					...body.vendor === void 0 ? {} : { vendor: body.vendor },
					...body.channel === void 0 ? {} : { channel: body.channel },
					...body.voices === void 0 ? {} : { voices: body.voices },
					...body.truncated === void 0 ? {} : { truncated: body.truncated },
			...
			async voiceRecommendHistory(limit = 20) {
				const body = await (await postJson(VOICES_API.recommendHistory.list, { limit })).json();
				return {
					ok: body.ok === true,
					...body.entries === void 0 ? {} : { entries: body.entries },
					...body.message === void 0 ? {} : { message: body.message }
				};
			}
			/** 删除一条 AI 音色推荐记录。 */
			async voiceRecommendHistoryRemove(id) {
				const body = await (await postJson(VOICES_API.recommendHistory.remove, { id })).json();
				return {
					ok: body.ok === true,
					...body.message === void 0 ? {} : { message: body.message }
				};
			...
			async voiceDelete(request) {
				const body = await (await postJson(VOICES_API.delete, request)).json();
				return {
					ok: body.ok === true,
					...body.message === void 0 ? {} : { message: body.message },
					...body.code === void 0 ? {} : { code: body.code }
			...
			async voiceRecommend(request) {
				const body = await (await postJson(VOICES_API.recommend, request)).json();
				return {
					ok: body.ok === true,
					...body.vendor === void 0 ? {} : { vendor: body.vendor },
					...body.channel === void 0 ? {} : { channel: body.channel },
					...body.requirement === void 0 ? {} : { requirement: body.requirement },
					...body.candidate_count === void 0 ? {} : { candidate_count: body.candidate_count },
					...body.top_k === void 0 ? {} : { top_k: body.top_k },
					...body.recommendations === void 0 ? {} : { recommendations: body.recommendations },
					...body.note === void 0 ? {} : { note: body.note },
					...body.code === void 0 ? {} : { code: body.code },
					...body.message === void 0 ? {} : { message: body.message }
				};
		...
		*/
		var BridgeScopeController = class {
			...
			/** Official SettingsScope mutation entry: several path ops in one revision-fenced mutate call. */
		...
		*/
		...
		function presetSupports(preset, key, mode) {
			...
			switch (key) {
				...
				case "sampleRate":
				case "bitrate": return p === PRESET_MINIMAX || mode === "sfx" && p === PRESET_ELEVENLABS;
		...
		const ELEVENLABS_SFX_SAMPLE_RATES = [
			"8000",
			"16000",
			"22050",
			"24000",
		...
		*  全局字段与「每模型覆盖」矩阵共用同一份枚举，避免两处选项不一致。 */
		function fieldOptionsFor(key, preset, mode) {
			const p = preset.toLowerCase();
			if (key === "format") {
				if (mode === "sfx") return SFX_FORMATS[p] ?? [
					"mp3",
					"wav",
					"pcm"
				];
				if (mode === "music") return MUSIC_FORMATS[p] ?? [
					"mp3",
					"wav",
					"pcm"
				];
			...
			if (key === "sampleRate") return p === PRESET_ELEVENLABS ? ELEVENLABS_SFX_SAMPLE_RATES : p === PRESET_MINIMAX ? SPECS.sampleRate.options : void 0;
			if (key === "bitrate") return p === PRESET_ELEVENLABS ? ELEVENLABS_SFX_BITRATES : p === PRESET_MINIMAX ? SPECS.bitrate.options : void 0;
		}
		...
		function globalFieldSpecs(mode, presets) {
			...
			if (mode === "tts") {
				...
				const supported = presets.length === 0 ? [[
					...
					"wav",
					"pcm"
				]] : presets.map((preset) => SFX_FORMATS[preset.toLowerCase()] ?? ["mp3", "wav"]);
				const intersect = supported.reduce((acc, cur) => acc.filter((item) => cur.includes(item)), supported[0] ?? ["mp3"]);
				list.push({
					...specOf("format"),
					options: intersect.length > 0 ? intersect : ["mp3"]
				});
				...
				if (all("sampleRate", SFX_KEYS) && presets.length > 0) list.push({
					...specOf("sampleRate"),
				...
				if (all("bitrate", SFX_KEYS) && presets.length > 0) list.push({
					...specOf("bitrate"),
					label: "码率（kbps）",
		...
		var audio_panel_module_css_default = {
			...
			"recVoiceRow": "Oo1fpq_recVoiceRow",
			...
			"recVoiceName": "Oo1fpq_recVoiceName",
		...
		*/
		...
		function StudioView(props) {
			...
			const effectiveConfig = useConfig$1(scope);
			...
			const [recommendChannelId, setRecommendChannelId] = (0, react.useState)("");
			(0, react.useEffect)(() => {
				if (channels.length === 0) return;
				if (recommendChannelId === "" || !channels.some((candidate) => candidate.id === recommendChannelId)) setRecommendChannelId(modelOptions.defaultChannelId ?? channels[0].id);
			}, [
				channels,
				modelOptions.defaultChannelId,
				recommendChannelId
			]);
			const [recommendTopK, setRecommendTopK] = (0, react.useState)(5);
			const [recommending, setRecommending] = (0, react.useState)(false);
			const [recommendError, setRecommendError] = (0, react.useState)(null);
			const [recommendResult, setRecommendResult] = (0, react.useState)(null);
			/** 右侧推荐记录（与音色管理页、Agent action=recommend 共用同一宿主存储）。 */
			const [recommendRecords, setRecommendRecords] = (0, react.useState)([]);
			const loadRecommendRecords = () => {
				api.voiceRecommendHistory(30).then((response) => setRecommendRecords(response.entries ?? [])).catch(() => {});
			};
			(0, react.useEffect)(() => {
			...
			const recommendVoice = async () => {
				...
				try {
					...
					} else setRecommendError(response.message ?? "音色推荐失败");
				} catch (err) {
			...
			const reuseRecommendVoice = (voiceId, channelId) => {
				const ttsModel = channels.find((item) => item.id === channelId)?.models.find((candidate) => candidate.category === "tts" || /tts|speech|voice|t2a/i.test(candidate.alias));
				setMode("tts");
				setVoice(voiceId);
				if (ttsModel !== void 0 && ttsModel.alias !== "") setModel(ttsModel.alias);
			};
			const removeRecommendRecord = async (id) => {
				await api.voiceRecommendHistoryRemove(id).catch(() => {});
				loadRecommendRecords();
			};
			...
			const effectiveSpecs = (0, react.useMemo)(() => {
				if (mode !== "sfx") return globalSpecs;
				if (!(fieldPresets.length > 0 && fieldPresets.every((preset) => preset.toLowerCase() === "elevenlabs"))) return globalSpecs;
			...
			const genMode = mode === "voice_recommend" ? "tts" : mode;
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: audio_panel_module_css_default.label,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: mode === "voice_design" ? "音色描述" : mode === "voice_recommend" ? "音色需求" : mode === "tts" ? "文本" : "提示词" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
									className: audio_panel_module_css_default.textarea,
									value: prompt,
							...
							mode !== "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							enhancePreview !== null && mode !== "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							mode === "voice_design" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							...
							] }) : mode === "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: audio_panel_module_css_default.label,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "厂商 / 渠道" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
										className: audio_panel_module_css_default.input,
										value: recommendChannelId,
										onChange: (event) => setRecommendChannelId(event.target.value),
										children: channels.map((channel) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
											value: channel.id,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: audio_panel_module_css_default.label,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "推荐数量" }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
										className: audio_panel_module_css_default.input,
										value: String(recommendTopK),
										onChange: (event) => setRecommendTopK(Number(event.target.value)),
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
												value: "3",
												children: "3 条"
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									className: audio_panel_module_css_default.hint,
									children: "按需求描述挑选：候选范围为该渠道全部可用音色（语言/口音等由描述自然约束），结果会记录到右侧「推荐记录」"
								})
							] }) : null,
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: audio_panel_module_css_default.formSection,
													...
													return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														...
														children: row.type === "select" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
															...
															children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
																value: "",
																children: "自动"
															}), (fieldOptionsFor(row.key, entry.preset, mode) ?? row.options ?? []).map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
																value: option,
																children: option
															}, option))]
														}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							...
							effectiveSpecs.some((spec) => spec.advanced !== true) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: audio_panel_module_css_default.formSection,
								children: "生成参数"
							}) : null,
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: audio_panel_module_css_default.formFields,
								children: effectiveSpecs.filter((spec) => spec.advanced !== true).map((spec) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							mode === "tts" && effectiveSpecs.some((spec) => spec.advanced === true) ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
								className: audio_panel_module_css_default.advanced,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", { children: "MiniMax 高级参数" }), effectiveSpecs.filter((spec) => spec.advanced === true).map((spec) => renderField(spec))]
							}) : null,
							mode !== "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: audio_panel_module_css_default.checkbox,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: mode === "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							recommendError !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: audio_panel_module_css_default.error,
								children: recommendError
							}) : null,
							recommending ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: audio_panel_module_css_default.resultEmpty,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: audio_panel_module_css_default.resultEmptyIcon,
									children: "🎤"
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "正在让模型从候选音色中挑选…（约 10-45 秒，结果会记录到右侧）" })]
							}) : null,
							!recommending && recommendResult !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												...
												title: recommendResult.requirement,
												children: recommendResult.requirement
											})]
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: audio_panel_module_css_default.historyMeta,
											children: [
												recommendResult.channel,
												" · 候选池 ",
												recommendResult.candidateCount,
												" 个音色 · 推荐 ",
												recommendResult.voices.length,
												" 条"
											]
										}),
										recommendResult.voices.map((voice, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: audio_panel_module_css_default.recVoiceRow,
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: audio_panel_module_css_default.recRank,
													children: index + 1
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
													className: audio_panel_module_css_default.recVoiceBody,
													children: [
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
															className: audio_panel_module_css_default.recVoiceHead,
															children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
																className: audio_panel_module_css_default.recVoiceName,
																title: voice.name,
																children: voice.name
															})
														}),
														voice.reason === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
															className: audio_panel_module_css_default.recReason,
															children: voice.reason
														}),
														/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
															className: audio_panel_module_css_default.historyMeta,
															children: [
																"ID: ",
																voice.voice_id,
																voice.language === void 0 ? "" : ` · ${voice.language}`,
																voice.locale === void 0 ? "" : ` · ${voice.locale}`,
																voice.accent === void 0 ? "" : ` · ${voice.accent}`,
																voice.gender === void 0 ? "" : ` · ${voice.gender}`,
																voice.age === void 0 ? "" : ` · ${voice.age}`,
																voice.use_case === void 0 ? "" : ` · ${voice.use_case}`
															]
														}),
														voice.descriptive === void 0 && (voice.labels === void 0 || Object.keys(voice.labels).length === 0) ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
															className: audio_panel_module_css_default.historyMeta,
															children: [voice.descriptive !== void 0 ? `标签 descriptive: ${voice.descriptive}` : "", voice.labels !== void 0 ? Object.entries(voice.labels).filter(([key]) => key !== "description" && key !== "descriptive").map(([key, value]) => `${key === "use_case" ? "用途" : key === "accent" ? "口音" : key === "gender" ? "性别" : key === "age" ? "年龄" : key}: ${value}`).join(" · ") : ""]
														}),
														voice.description === void 0 || voice.description === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
															className: audio_panel_module_css_default.recDescription,
															children: voice.description
														}),
														voice.preview_url === void 0 || voice.preview_url === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AudioPlayer, {
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													type: "button",
													className: audio_panel_module_css_default.ghostButton,
													onClick: () => reuseRecommendVoice(voice.voice_id, recommendResult.channelId),
													children: "用此音色生成"
												})
											]
										}, `${voice.source}:${voice.voice_id}`))
									]
								})
							}) : null,
							!recommending && recommendResult === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: audio_panel_module_css_default.resultEmpty,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: audio_panel_module_css_default.resultEmptyIcon,
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "在左侧描述想要的音色，点「✨ AI 推荐音色」" }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
										className: audio_panel_module_css_default.resultEmptyHint,
										children: "模型会从该渠道候选池里挑最匹配的 top-k 音色；每次推荐自动记录到右侧「推荐记录」"
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: audio_panel_module_css_default.ghostButton,
						...
						] }) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [error !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: audio_panel_module_css_default.error,
							children: error
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("aside", {
						...
						children: mode === "voice_recommend" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: audio_panel_module_css_default.historyHeader,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
								className: audio_panel_module_css_default.historyTitle,
								children: "推荐记录"
							})
						}), recommendRecords.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: audio_panel_module_css_default.historyEmpty,
							children: "还没有推荐记录——在左侧输入音色需求，点「✨ AI 推荐音色」后自动记录在这里"
						}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: audio_panel_module_css_default.historyList,
							children: recommendRecords.map((record) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", {
										...
										children: [
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												...
												children: formatClock(record.createdAt)
											})
										]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									record.recommendations.map((voice, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: audio_panel_module_css_default.recVoiceRow,
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: audio_panel_module_css_default.recRank,
												...
												children: [
													...
													/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
														...
														children: [
															"ID: ",
															voice.voice_id,
															voice.descriptive === void 0 ? "" : ` · ${voice.descriptive}`,
															voice.accent === void 0 ? "" : ` · ${voice.accent}`,
															voice.gender === void 0 ? "" : ` · ${voice.gender}`,
															voice.age === void 0 ? "" : ` · ${voice.age}`
														]
													...
													voice.description === void 0 || voice.description === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
														className: audio_panel_module_css_default.recDescription,
														children: voice.description.length > 120 ? `${voice.description.slice(0, 119)}…` : voice.description
													})
												]
											}),
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											title: "删除这条推荐记录",
											onClick: () => void removeRecommendRecord(record.id),
											children: "✕"
										})
									})
								]
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/shimingming/Projects_code/dsh-audiogen/src/client/voices.module.css.mjs
		const css$3 = ".FjxQqG_view{flex-direction:column;gap:10px;height:100%;min-height:0;padding:14px 16px;display:flex;overflow:hidden}.FjxQqG_toolbar{grid-template-columns:repeat(auto-fit,minmax(150px,1fr));align-items:end;gap:8px;display:grid}.FjxQqG_field{flex-direction:column;gap:4px;min-width:0;display:flex}.FjxQqG_fieldLabel{color:var(--dsw-alias-fg-l3,#6b7280);font-size:12px}.FjxQqG_input,.FjxQqG_select{border:1px solid var(--dsw-alias-border-l2,#d1d5db);background:var(--dsw-alias-bg,#fff);width:100%;min-height:30px;color:var(--dsw-alias-fg,#111827);border-radius:8px;outline:none;padding:5px 8px;font-size:13px}.FjxQqG_input:focus,.FjxQqG_select:focus{border-color:var(--dsw-accent,#2563eb)}.FjxQqG_primaryBtn{background:var(--dsw-accent,#2563eb);color:#fff;cursor:pointer;border:none;border-radius:8px;align-items:center;gap:6px;height:30px;padding:0 12px;font-size:13px;display:inline-flex}.FjxQqG_primaryBtn:hover:not(:disabled){filter:brightness(1.1)}.FjxQqG_primaryBtn:disabled{opacity:.6;cursor:not-allowed}.FjxQqG_advanced{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f9fafb);border-radius:8px;padding:6px 10px}.FjxQqG_advancedSummary{color:var(--dsw-alias-fg-l3,#6b7280);cursor:pointer;user-select:none;font-size:12px}.FjxQqG_advancedGrid{grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-top:8px;display:grid}.FjxQqG_checkRow{color:var(--dsw-alias-fg-l2,#374151);align-items:center;gap:6px;min-height:30px;font-size:12px;display:flex}.FjxQqG_stateLine{align-items:center;gap:10px;min-height:18px;display:flex}.FjxQqG_stateNote{color:var(--dsw-alias-fg-l3,#6b7280);font-size:12px}.FjxQqG_stateNote[data-error]{color:#dc2626}.FjxQqG_count{color:var(--dsw-alias-fg-l3,#6b7280);font-size:12px}.FjxQqG_list{flex-direction:column;flex:1;gap:8px;min-height:0;padding-right:2px;display:flex;overflow-y:auto}.FjxQqG_card{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg,#fff);border-radius:10px;flex-direction:column;gap:6px;padding:10px 12px;display:flex}.FjxQqG_cardHead{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.FjxQqG_voiceName{color:var(--dsw-alias-fg,#111827);text-overflow:ellipsis;white-space:nowrap;max-width:60%;font-size:13.5px;font-weight:600;overflow:hidden}.FjxQqG_badge{background:var(--dsw-alias-bg-layer-2,#f3f4f6);color:var(--dsw-alias-fg-l3,#6b7280);border-radius:999px;padding:2px 8px;font-size:11px}.FjxQqG_badge[data-source=custom],.FjxQqG_badge[data-source=owned]{color:#1d4ed8;background:#dbeafe}.FjxQqG_meta{color:var(--dsw-alias-fg-l3,#6b7280);flex-wrap:wrap;gap:8px;font-size:11.5px;display:flex}.FjxQqG_voiceId{word-break:break-all;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.FjxQqG_description{color:var(--dsw-alias-fg-l2,#374151);max-height:64px;margin:0;font-size:12.5px;line-height:1.5;overflow:hidden}.FjxQqG_preview{max-width:340px}.FjxQqG_actions{align-items:center;gap:8px;display:flex}.FjxQqG_useBtn,.FjxQqG_deleteBtn{cursor:pointer;border:1px solid var(--dsw-alias-border-l2,#d1d5db);background:var(--dsw-alias-bg,#fff);height:28px;color:var(--dsw-alias-fg,#111827);border-radius:8px;align-items:center;gap:5px;padding:0 10px;font-size:12.5px;display:inline-flex}.FjxQqG_useBtn:hover{border-color:var(--dsw-accent,#2563eb);color:var(--dsw-accent,#2563eb)}.FjxQqG_deleteBtn{color:#dc2626}.FjxQqG_deleteBtn:hover:not(:disabled){background:#fef2f2;border-color:#dc2626}.FjxQqG_deleteBtn:disabled{opacity:.5;cursor:not-allowed}.FjxQqG_empty{border:1px dashed var(--dsw-alias-border-l2,#d1d5db);color:var(--dsw-alias-fg-l3,#6b7280);border-radius:10px;justify-content:center;align-items:center;gap:8px;padding:28px 12px;font-size:12.5px;display:flex}.FjxQqG_recommend{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f9fafb);border-radius:10px;flex-direction:column;gap:8px;padding:10px 12px;display:flex}.FjxQqG_recommendHead{color:var(--dsw-accent,#2563eb);flex-wrap:wrap;align-items:center;gap:6px;display:flex}.FjxQqG_recommendTitle{color:var(--dsw-alias-fg,#111827);font-size:13px;font-weight:600}.FjxQqG_recommendHint{color:var(--dsw-alias-fg-l3,#6b7280);font-size:11.5px}.FjxQqG_recommendBody{flex-direction:column;gap:8px;display:flex}.FjxQqG_recommendInput{border:1px solid var(--dsw-alias-border-l2,#d1d5db);background:var(--dsw-alias-bg,#fff);width:100%;min-height:48px;color:var(--dsw-alias-fg,#111827);resize:vertical;border-radius:8px;outline:none;padding:7px 9px;font-family:inherit;font-size:13px}.FjxQqG_recommendInput:focus{border-color:var(--dsw-accent,#2563eb)}.FjxQqG_recommendControls{align-items:end;gap:8px;display:flex}.FjxQqG_recommendResults{flex-direction:column;gap:8px;margin-top:2px;display:flex}.FjxQqG_recommendCard{border-color:var(--dsw-accent,#2563eb);background:color-mix(in srgb, var(--dsw-accent,#2563eb) 4%, var(--dsw-alias-bg,#fff))}.FjxQqG_rank{background:var(--dsw-accent,#2563eb);color:#fff;border-radius:999px;flex:none;justify-content:center;align-items:center;width:20px;height:20px;font-size:11px;font-weight:600;display:inline-flex}.FjxQqG_reason{color:var(--dsw-accent,#2563eb);margin:0;font-size:12.5px;line-height:1.5}.FjxQqG_recordBox{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f9fafb);border-radius:10px;padding:8px 12px}.FjxQqG_recordSummary{cursor:pointer;color:var(--dsw-alias-fg,#111827);user-select:none;align-items:center;gap:6px;font-size:13px;font-weight:600;display:flex}.FjxQqG_recordHint{color:var(--dsw-alias-fg-l3,#6b7280);font-size:11.5px;font-weight:400}.FjxQqG_recordList{flex-direction:column;gap:8px;margin-top:8px;display:flex}.FjxQqG_recordCard{border-left:3px solid var(--dsw-accent,#2563eb)}.FjxQqG_recordVoices{flex-direction:column;gap:4px;margin-top:6px;display:flex}.FjxQqG_recordVoiceRow{border-radius:6px;flex-wrap:wrap;align-items:center;gap:8px;padding:4px 2px;display:flex}.FjxQqG_recordVoiceRow:hover{background:color-mix(in srgb, var(--dsw-accent,#2563eb) 5%, transparent)}.FjxQqG_recordVoiceName{color:var(--dsw-alias-fg,#111827);text-overflow:ellipsis;white-space:nowrap;max-width:220px;font-size:13px;font-weight:500;overflow:hidden}.FjxQqG_reasonInline{color:var(--dsw-accent,#2563eb);font-size:12px}";
		const tagId$3 = "dsh-audiogen/voices.module.css";
		...
		var voices_module_css_default = {
			"primaryBtn": "FjxQqG_primaryBtn",
			"recommend": "FjxQqG_recommend",
			"recordCard": "FjxQqG_recordCard",
			"reasonInline": "FjxQqG_reasonInline",
			"toolbar": "FjxQqG_toolbar",
			"voiceId": "FjxQqG_voiceId",
			"preview": "FjxQqG_preview",
			"recommendControls": "FjxQqG_recommendControls",
			"recommendTitle": "FjxQqG_recommendTitle",
			"select": "FjxQqG_select",
			"recordVoiceName": "FjxQqG_recordVoiceName",
			"rank": "FjxQqG_rank",
			"count": "FjxQqG_count",
			"recommendHead": "FjxQqG_recommendHead",
			"recommendHint": "FjxQqG_recommendHint",
			"recommendResults": "FjxQqG_recommendResults",
			"cardHead": "FjxQqG_cardHead",
			"meta": "FjxQqG_meta",
			"recommendCard": "FjxQqG_recommendCard",
			"list": "FjxQqG_list",
			"recordSummary": "FjxQqG_recordSummary",
			"stateLine": "FjxQqG_stateLine",
			"card": "FjxQqG_card",
			"fieldLabel": "FjxQqG_fieldLabel",
			"field": "FjxQqG_field",
			"useBtn": "FjxQqG_useBtn",
			"recordList": "FjxQqG_recordList",
			"advancedSummary": "FjxQqG_advancedSummary",
			"badge": "FjxQqG_badge",
			"recommendInput": "FjxQqG_recommendInput",
			"input": "FjxQqG_input",
			"advancedGrid": "FjxQqG_advancedGrid",
			"voiceName": "FjxQqG_voiceName",
			"actions": "FjxQqG_actions",
			"advanced": "FjxQqG_advanced",
			"checkRow": "FjxQqG_checkRow",
			"recordBox": "FjxQqG_recordBox",
			"recordVoiceRow": "FjxQqG_recordVoiceRow",
			"description": "FjxQqG_description",
			"deleteBtn": "FjxQqG_deleteBtn",
			"view": "FjxQqG_view",
			"reason": "FjxQqG_reason",
			"recordHint": "FjxQqG_recordHint",
			"empty": "FjxQqG_empty",
			"recordVoices": "FjxQqG_recordVoices",
			"recommendBody": "FjxQqG_recommendBody",
			"stateNote": "FjxQqG_stateNote"
		};
		//#endregion
		//#region src/client/voices-view.tsx
		/**
		...
		*/
		...
		function VoicesView(props) {
			const { api, scope, showToast, onReuseVoice } = props;
			const cfg = useConfig(scope);
			const channels = cfg?.channels ?? [];
			const channelOptions = (0, react.useMemo)(() => channels.filter((channel) => channel.apiUrl.trim() !== ""), [channels]);
			...
			const [useCase, setUseCase] = (0, react.useState)("");
			const [accent, setAccent] = (0, react.useState)("");
			const [gender, setGender] = (0, react.useState)("");
			const [age, setAge] = (0, react.useState)("");
			const [locale, setLocale] = (0, react.useState)("");
			const [category, setCategory] = (0, react.useState)("");
			const [sort, setSort] = (0, react.useState)("");
			const [featured, setFeatured] = (0, react.useState)(false);
			const [freeUsersAllowed, setFreeUsersAllowed] = (0, react.useState)(false);
			...
			const [recordsOpen, setRecordsOpen] = (0, react.useState)(false);
			...
			const load = async () => {
				if (channels.find((item) => item.id === channelId) === void 0) return;
				setLoading(true);
				setError(null);
				setNote(null);
				try {
			...
			const remove = async (voice) => {
				...
				try {
					const response = await api.voiceDelete({
						channel: channelId,
						voice_id: voice.voice_id,
						confirm: true
					});
					if (response.ok) showToast(`已删除音色 ${voice.name}`);
					else showToast(response.message ?? "删除失败");
				} catch (err) {
			...
			const reuse = (voice) => {
				const ttsModel = channels.find((item) => item.id === channelId)?.models.find((model) => model.category === "tts" || /tts|speech|voice|t2a/i.test(model.alias));
				onReuseVoice({
					mode: "tts",
					voiceId: voice.voice_id,
					...ttsModel === void 0 || ttsModel.alias === "" ? {} : { model: ttsModel.alias }
				});
			};
			/** 从推荐记录里复用音色：切到记录对应的渠道，再回填 TTS 表单。 */
			const reuseFromRecord = (record, voice) => {
				if (record.channel_id !== void 0 && record.channel_id !== "" && channels.some((item) => item.id === record.channel_id)) setChannelId(record.channel_id);
				const ttsModel = channels.find((item) => item.id === (record.channel_id ?? channelId))?.models.find((model) => model.category === "tts" || /tts|speech|voice|t2a/i.test(model.alias));
				onReuseVoice({
					mode: "tts",
					voiceId: voice.voice_id,
					...ttsModel === void 0 || ttsModel.alias === "" ? {} : { model: ttsModel.alias }
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: voices_module_css_default.view,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: voices_module_css_default.toolbar,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: voices_module_css_default.field,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: voices_module_css_default.fieldLabel,
									children: "厂商 / 渠道"
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
									className: voices_module_css_default.select,
									value: channelId,
									onChange: (event) => setChannelId(event.target.value),
									children: channelOptions.map((channel) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: channel.id,
										children: channel.name
									}, channel.id))
								})]
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									className: voices_module_css_default.input,
									value: keyword,
									onChange: (event) => setKeyword(event.target.value),
									placeholder: "名字/描述/口音…"
								})]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: voices_module_css_default.field,
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: voices_module_css_default.primaryBtn,
								onClick: () => void load(),
								disabled: loading,
								children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: voices_module_css_default.stateNote,
						children: "按需求描述（prompt）推荐音色请使用生成页左侧「🎤 音色推荐」模式；这里仅浏览/筛选/删除音色，下方为历次 AI 推荐记录。"
					}),
					records.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
						className: voices_module_css_default.recordBox,
						open: recordsOpen,
						onToggle: (event) => setRecordsOpen(event.target.open),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", {
							className: voices_module_css_default.recordSummary,
							children: [
								...
								"）",
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: voices_module_css_default.recordHint,
									children: "每次推荐自动记录，点「用此音色生成」可直接回填 TTS 表单"
								})
							]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: voices_module_css_default.recordList,
							children: records.map((record) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: `${voices_module_css_default.card} ${voices_module_css_default.recordCard}`,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: voices_module_css_default.cardHead,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: voices_module_css_default.voiceName,
											title: record.requirement,
											children: record.requirement
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: voices_module_css_default.badge,
											"data-source": record.vendor,
											children: record.channel
										})]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: voices_module_css_default.meta,
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: new Date(record.createdAt).toLocaleString() }),
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["候选池 ", record.candidate_count] }),
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: voices_module_css_default.recordVoices,
										children: record.recommendations.map((voice, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: voices_module_css_default.recordVoiceRow,
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: voices_module_css_default.rank,
													children: index + 1
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: voices_module_css_default.recordVoiceName,
													title: voice.name,
													children: voice.name
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
													className: voices_module_css_default.voiceId,
													title: voice.voice_id,
													children: ["ID: ", voice.voice_id]
												}),
												voice.descriptive === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
													className: voices_module_css_default.reasonInline,
													children: ["descriptive: ", voice.descriptive]
												}),
												voice.reason === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: voices_module_css_default.reasonInline,
													children: voice.reason
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
													type: "button",
													className: voices_module_css_default.useBtn,
													onClick: () => reuseFromRecord(record, voice),
													children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MicIcon, {}), " 用此音色生成"]
												})
											]
										}, `${record.id}:${voice.voice_id}`))
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: voices_module_css_default.actions,
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
											type: "button",
											className: voices_module_css_default.deleteBtn,
											onClick: () => void removeRecord(record.id),
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(TrashIcon, {}), " 删除记录"]
										})
									})
								]
							}, record.id))
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
						className: voices_module_css_default.advanced,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", {
							className: voices_module_css_default.advancedSummary,
							children: "官方共享库筛选（ElevenLabs /v1/shared-voices，其它渠道无效）"
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: voices_module_css_default.advancedGrid,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: voices_module_css_default.field,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: voices_module_css_default.fieldLabel,
										children: "用途（use_case）"
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										className: voices_module_css_default.input,
										value: useCase,
										onChange: (event) => setUseCase(event.target.value),
										placeholder: "characters_animation / narration…"
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: voices_module_css_default.field,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: voices_module_css_default.checkRow,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										type: "checkbox",
										checked: featured,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										type: "checkbox",
										checked: freeUsersAllowed,
										onChange: (event) => setFreeUsersAllowed(event.target.checked)
									}), " 仅免费用户可用"]
								})
							]
						})]
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: voices_module_css_default.stateLine,
						children: [
							loading ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: voices_module_css_default.stateNote,
								children: "正在拉取音色列表…"
							}) : null,
							!loading && error !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: voices_module_css_default.stateNote,
								"data-error": true,
								children: ["⚠ ", error]
							}) : null,
							!loading && note !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: voices_module_css_default.stateNote,
								children: note
							}) : null,
							!loading && truncated ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: voices_module_css_default.stateNote,
								children: [
									"已截断：仅显示前 ",
									voices.length,
							...
							!loading && error === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: voices_module_css_default.count,
								children: [voices.length, " 个音色"]
							}) : null
						]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: voices_module_css_default.list,
						children: [voices.map((voice) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: voices_module_css_default.card,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: voices_module_css_default.cardHead,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										...
										children: [SOURCE_LABELS[voice.source], voice.deletable ? "" : " · 只读"]
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: voices_module_css_default.meta,
									children: [
										...
										voice.language === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: voice.language }),
										voice.accent === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: voice.accent }),
										voice.gender === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: voice.gender }),
										voice.age === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: voice.age }),
										voice.use_case === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: voice.use_case })
									]
								}),
								voice.description === void 0 || voice.description === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									className: voices_module_css_default.description,
									children: voice.description
								}),
								voice.preview_url === void 0 || voice.preview_url === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: voices_module_css_default.preview,
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AudioPlayer, {
										src: voice.preview_url,
										compact: true
									})
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										...
										onClick: () => reuse(voice),
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MicIcon, {}), " 用此音色生成"]
									}), voice.deletable ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										type: "button",
										className: voices_module_css_default.deleteBtn,
										...
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)(TrashIcon, {}),
											" ",
											deletingId === voice.voice_id ? "删除中…" : "删除"
										]
									}) : null]
								})
							]
						}, `${voice.source}:${voice.voice_id}`)), !loading && error === null && voices.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: voices_module_css_default.empty,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CheckIcon, {}), " 没有匹配的音色——请调整筛选条件，或该渠道不支持音色库接口（部分网关不映射 /v1/voices）。"]
						}) : null]
					})
				]
			});
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MicIcon, {}), " 音色管理"]
								})
							]
						})]
					}),
					tab === "studio" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(StudioView, {
						...
						showToast
					}) : tab === "library" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(LibraryView, {
						api,
						revision: libraryRev,
						showToast,
					...
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(VoicesView, {
						api,
						scope,
						showToast,
